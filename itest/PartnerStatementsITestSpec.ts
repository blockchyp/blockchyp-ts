...
import { PartnerStatementListRequest } from '../index';
import { PartnerStatementListResponse } from '../index';
  ...
  it('can list partner statements', function (done) {
    ...
    setTimeout(async function () {
      try {
        ...
        const request = new PartnerStatementListRequest();
        request.test = true;

        const httpResponse = await client.partnerStatements(request)
        const response: PartnerStatementListResponse = httpResponse.data;
        // response assertions
