...
import { TokenMetadataRequest } from '../index';
import { TokenMetadataResponse } from '../index';
  ...
  it('can retrieve token metadata', function (done) {
    ...
    setTimeout(async function () {
      try {
        ...
        // setup request object
        const request = new TokenMetadataRequest();
        request.token = setupResponse.token ?? null;

        const httpResponse = await client.tokenMetadata(request)
        const response: TokenMetadataResponse = httpResponse.data;
        // response assertions
