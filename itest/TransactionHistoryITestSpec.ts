...
import { TransactionHistoryRequest } from '../index';
import { TransactionHistoryResponse } from '../index';
  ...
  it('can retrive transaction history', function (done) {
    ...
    setTimeout(async function () {
      try {
        ...
        // setup request object
        const request = new TransactionHistoryRequest();
        request.maxResults = 10;

        const httpResponse = await client.transactionHistory(request)
        const response: TransactionHistoryResponse = httpResponse.data;
        // response assertions
